...
/*
function getCurrentContentTypes(){
  var contentTypes = adminUtils.getCurrentContentTypes();
  
  //showObject(contentTypes);
  ...
  for(var a=0;a<contentTypes.length;a++){
    selected = '';
    js = ' onclick="javascript:document.location=\'' + Request.ServerVariables("SCRIPT_NAME") + '?contentid=' + contentTypes[a].id + '\';"'
    if(contentTypes[a].id == parseInt(Request.QueryString("contentid"))){
      selected = ' checked="checked"';
    }
    out += '\t<li><input type="radio" name="contentid" value="' + contentTypes[a].id +'"' + selected + js + '>'+contentTypes[a].name +' (' + contentTypes[a].description + ')</li>\n';
  }
  out += "</ul>\n";
  return out;
}
...
function getCurrentContentTypesDropdown(cTypeList){
